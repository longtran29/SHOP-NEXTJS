...
function ReviewProduct(props) {
  ...
  const submitReview = async () => {
    ...
    const resGet = await fetch(`${NEXT_API}/api/review?action=post_review`, {
      method: "POST",
      headers: {    
    ...
    if (!resGet.ok) {
      ...
      console.log("Review ", JSON.stringify(dataGet.reviews));
  ...
  useEffect(() => {
    (async () => {
      const rest = await fetch(
        `${NEXT_API}/api/review?action=get_review&productId=${data.id}`,
        {
          method: "GET",
        }
      );
