...
const ProductDetail = ({ foundedProd }) => {
  const responsive = {
    0: { items: 1 },
    568: { items: 2 },
    1024: { items: 4 },
...
export async function getStaticPaths() {
  // Await the data from getAllProduct()
  const listProds = await getAllProduct();

  console.log("Ds prods 2 ", JSON.stringify(listProds));

  const paths = listProds.map((prod) => {
    return {
...
export async function getStaticProps({ params }) {
  ...
  const foundedProd = listProds.find((product) => product.id == prodId);
